...
@section('content')
	<div class="flex flex-col gap-10 px-5 mt-5">
		<div class="breadcrumb flex items-center gap-[30px]">
			...
			<span class="text-[#7F8190] last:text-[#0A090B]">/</span>
			<span class="text-[#7F8190] last:text-[#0A090B] last:font-semibold ">Course Details</span>
		</div>
	</div>
	<div class="header ml-[70px] pr-[70px] w-[940px] flex items-center justify-between mt-10">
		...
		<div class="relative">
			...
			<div
				...
				<a href="{{ route('dashboard.courses.course_student.create', $course) }}" class="flex gap-[10px] items-center">
					<div class="w-5 h-5">
						<img src="{{ asset('assets/images/icons/profile-2user-outline.svg') }}" alt="icon">
					</div>
					<span class="font-semibold text-sm">Add Students</span>
				</a>
				<a href="{{ route('dashboard.courses.course_student.index', $course) }}" class="flex gap-[10px] items-center">
					<div class="w-5 h-5">
						<img src="{{ asset('assets/images/icons/profile-2user-outline.svg') }}" alt="icon">
					</div>
	...
	<div id="course-test" class="mx-[70px] w-[870px] mt-[30px]">
		...
		<div class="flex flex-col gap-[30px] mt-2">
			<a href="{{ route('dashboard.courses.create.question', $course) }}"
			...
			@forelse ($course->questions as $questions)
				<div class="question-card w-full flex items-center justify-between p-4 border border-[#EEEEEE] rounded-[20px]">
					...
					<div class="flex items-center gap-[14px]">
						<a href="{{ route('dashboard.course_questions.edit', $questions) }}"
							class="bg-[#0A090B] p-[14px_30px] rounded-full text-white font-semibold">
							Edit
						</a>

						<form action="{{ route('dashboard.course_questions.destroy', $questions) }}" method="POST">
							@csrf
							@method('DELETE')
			...
			@empty
				<div class="question-card w-full flex items-center justify-between p-4 border border-[#EEEEEE] rounded-[20px]">
					<div class="text-center w-full">
						<p class="text-[#7F8190] italic">No Data</p>
					</div>
				</div>
			@endforelse
		</div>
