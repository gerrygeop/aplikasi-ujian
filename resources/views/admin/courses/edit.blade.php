...
@section('content')
	<div class="flex flex-col gap-10 px-5 mt-5">
		<div class="breadcrumb flex items-center gap-[30px]">
			...
			<a href="{{ route('dashboard.courses.index') }}" class="text-[#7F8190] last:text-[#0A090B] last:font-semibold">
				Manage Courses
			</a>
			<span class="text-[#7F8190] last:text-[#0A090B]">/</span>
	...
	<form action="{{ route('dashboard.courses.update', $course) }}" method="POST" enctype="multipart/form-data"
		...
		<div class="flex gap-5 items-center">
			...
			<div
				...
				<span
					class="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 text-center font-semibold text-sm text-[#7F8190]">
					Icon <br />Course
				</span>
			</div>
		...
		<div class="flex flex-col gap-[10px]">
			...
			<div
				class="flex items-center w-[500px] h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-1 focus-within:border-[#0A090B]">
				<div class="mr-[14px] w-6 h-6 flex items-center justify-center overflow-hidden">
					<img src="{{ asset('assets/images/icons/note-favorite-outline.svg') }}" class="w-full h-full object-contain"
						alt="icon" />
				</div>
				<input type="text" value="{{ old('name', $course->name) }}"
					class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full focus:ring-0 border-0 outline-none"
		...
		<div class="group/category flex flex-col gap-[10px]">
			...
			<div
				class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-1 focus-within:border-[#0A090B]">
				...
				<select id="category"
					class="pl-1 font-semibold focus:outline-none focus:ring-0 border-0 w-full text-[#0A090B] invalid:text-[#7F8190] invalid:font-normal appearance-none bg-[url('{{ asset('assets/images/icons/arrow-down.svg') }})] bg-no-repeat bg-right"
					name="category_id" required>
					<option value="" disabled selected hidden>Choose one of category</option>
					@forelse ($categories as $category)
						<option value="{{ $category->id }}" class="font-semibold" @selected($course->category_id == $category->id)>
							{{ $category->name }}
						</option>
					@empty
					@endforelse
		...
		<div class="flex flex-col gap-[10px]">
			...
			<div class="flex gap-5 items-center">
				<a href="#"
					class="group relative flex flex-col w-full items-center gap-5 p-[30px_16px] border border-[#EEEEEE] rounded-[30px] transition-all duration-300 aria-checked:border-1 aria-checked:border-[#0A090B]"
		...
		<div class="flex flex-col gap-[10px]">
			...
			<div class="flex gap-[10px] items-center">
				<a href="#"
					class="group relative flex w-full items-center gap-[14px] p-[14px_16px] border border-[#EEEEEE] rounded-full transition-all duration-300 aria-checked:border-1 aria-checked:border-[#0A090B]"
				...
				<a href="#"
					class="group relative flex w-full items-center gap-[14px] p-[14px_16px] border border-[#EEEEEE] rounded-full transition-all duration-300 aria-checked:border-1 aria-checked:border-[#0A090B] disabled:border-[#EEEEEE]"
		...
		<div class="group/access flex flex-col gap-[10px]">
			...
			<div
				class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-1 focus-within:border-[#0A090B]">
				<div class="mr-[10px] w-6 h-6 flex items-center justify-center overflow-hidden">
					<img src="{{ asset('assets/images/icons/security-user.svg') }}" class="w-full h-full object-contain"
						alt="icon" />
				</div>
				...
				<select id="access"
					...
					<option value="invite only" class="font-semibold">Invite Only</option>
				</select>
			</div>
		</div>
		...
		<div class="flex items-center gap-5">
			<a href=""
				class="w-full h-[52px] p-[14px_20px] bg-[#0A090B] rounded-full font-semibold text-white transition-all duration-300 text-center">
				Add to Draft
			</a>
			<button type="submit"
