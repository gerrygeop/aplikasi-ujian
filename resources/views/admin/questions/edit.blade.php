...
@section('content')
	...
	<form action="{{ route('dashboard.course_questions.update', $courseQuestion) }}" method="POST" id="add-question"
		...
		<div class="flex flex-col gap-[10px]">
			...
			<div
				...
				<input type="text"
					...
					placeholder="Write the question" name="question" value="{{ $courseQuestion->question }}" required>
			</div>
		</div>
		...
		<div class="flex flex-col gap-[10px]">
			...
			@foreach ($courseQuestion->answers as $index => $answers)
				<div class="flex items-center gap-4">
					<div
						...
						<input type="text"
							class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none border-0 focus:ring-0"
							placeholder="Write better answer option" name="answers[]" value="{{ $answers->answer }}" required>
					</div>

					<label class="font-semibold flex items-center gap-[10px]">
						<input type="radio" name="correct_answer" value={{ $index }} @checked(old('correct_answer', $answers->is_correct))
