		...
		@forelse ($courses as $course)
			<div class="list-items flex flex-nowrap justify-between pr-10">
				<div class="flex shrink-0 w-[300px]">
					<div class="flex items-center gap-4">
						<div class="w-16 h-16 flex shrink-0 overflow-hidden rounded-full">
							<img src="{{ Storage::url($course->cover) }}" class="object-cover" alt="{{ $course->slug }}">
						</div>
				...
				<div class="flex shrink-0 w-[120px] items-center">
					@if ($course->nextQuestion != null)
						<a href="{{ route('dashboard.learning.course', ['course' => $course, 'question' => $course->nextQuestion]) }}"
					...
					@else
						<a href="{{ route('dashboard.learning.raport.course', $course) }}"
							class="w-full h-[41px] p-[10px_20px] bg-[#21ca45] rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">
							Result
						</a>
					@endif
				</div>
